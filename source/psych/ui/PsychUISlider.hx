	...
	override function update(elapsed:Float)
	{
		...
		if(FlxG.mouse.justMoved || FlxG.mouse.justPressed || forceNextUpdate)
		{
			...
			if(movingHandle)
			{
				...
				value = Math.max(min, Math.min(max, FlxMath.remapToRange(#if (flixel < "5.9.0") FlxG.mouse.getPositionInCameraView(camera).x #else FlxG.mouse.getViewPosition(camera).x #end, bar.x, bar.x + bar.width, min, max)));
