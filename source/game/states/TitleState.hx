...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		/*#if (polymod && !html5)
		if (sys.FileSystem.exists('contents/')) {
			var folders:Array<String> = [];
			for (file in sys.FileSystem.readDirectory('contents/')) {
				var path = haxe.io.Path.join(['contents/', file]);
		...
		if(ClientPrefs.checkForUpdates && !closedState) {
			...
			http.onData = function (data:String)
			{
				...
				var curVersion:String = MainMenuState.ccEngineVersion.trim();
	...
	function startIntro()
	{
		...
		var easterEgg:String = FlxG.save.data.psychDevsEasterEgg ?? ''; //html5 fix
